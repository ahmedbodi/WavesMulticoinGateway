...
class BitcoinTransactionService(gw.TransactionService):
    ...
    def __init__(self, proxy: AuthServiceProxy, chain_query_service: BitcoinChainQueryService, min_optimized_amount: Decimal = Decimal(0.0000001)) -> None:
        self.proxy = proxy
        self.chain_query_service = chain_query_service
    ...
    def send_coin(self, attempt: gw.TransactionAttempt, secret: Optional[str] = None) -> gw.Transaction:
        ...
        unspents = self.proxy.listunspent(None, None, [attempt.sender])
        ...
        raw_transaction = self.proxy.createrawtransaction(optimized_unspents, outputs)
        signed_transaction = self.proxy.signrawtransaction(raw_transaction)
        tx = self.proxy.sendrawtransaction(signed_transaction['hex'])

        self._lock.release()

        return self.chain_query_service.get_transaction(tx)
